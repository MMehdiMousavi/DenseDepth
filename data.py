...
def get_nyu_data(batch_size, nyu_data_zipfile='Morse_13K.zip'):
    data = extract_zip(nyu_data_zipfile)

    nyu2_train = list((row.split(',') for row in (data['ue4_train.csv']).decode("utf-8").split('\r\n') if len(row) > 0))
    nyu2_test = list((row.split(',') for row in (data['ue4_test.csv']).decode("utf-8").split('\r\n') if len(row) > 0))

    # for i in range(0,len(nyu2_train)):
    #     nyu2_train[i][1]=nyu2_train[i][1][0:len(nyu2_train[i][1])-1]
    #     print(nyu2_train)
    #
    # for i in range(0,len(nyu2_test)):
    #     nyu2_test[i][1]=nyu2_test[i][1][0:len(nyu2_test[i][1])-1]
    # print(nyu2_test)
    # sys.exit()
...
def get_unreal_train_test_data(batch_size):
    ...
    train_generator = Unreal_BasicAugmentRGBSequence(data, unreal_train, batch_size=batch_size, shape_rgb=shape_rgb,
                                                     shape_depth=shape_depth)
    test_generator = Unreal_BasicAugmentRGBSequence(data, unreal_test, batch_size=batch_size, shape_rgb=shape_rgb,
                                                    shape_depth=shape_depth, is_skip_policy=True)
...
class Unreal_BasicAugmentRGBSequence(Sequence):
    def __init__(self, data, dataset, batch_size, shape_rgb, shape_depth, is_flip=False, is_addnoise=False,
                 is_erase=False, is_skip_policy=False):
        self.data = data
        self.dataset = dataset
        self.policy = BasicPolicy(color_change_ratio=0.50, mirror_ratio=0.50, flip_ratio=0.0 if not is_flip else 0.2,
                                  add_noise_peak=0 if not is_addnoise else 20,
    ...
    def __getitem__(self, idx, is_apply_policy=True):
        ...
        for i in range(batch_x.shape[0]):
            ...
            depth_sample = resize(depth_sample, (self.shape_depth[1], self.shape_depth[2]), preserve_range=True,
